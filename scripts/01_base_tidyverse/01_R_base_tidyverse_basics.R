...
class(num)
class(chr)
class(lgl) # class() 看"类型"（面向使用者的分类）
typeof(num)
typeof(int) # typeof() 看"底层存储类型"，比 class() 更底层
class(dte)
print(dte) # 日期打印出来是 2026-09-14，但本质是数字
...
is.numeric(num)
is.character(chr) # is.* 系列：判断类型，返回 TRUE / FALSE
is.na(NA)
is.null(NULL) # 判断缺失 / 判断空对象（NA 与 NULL 是两回事，见第 02 节）
...
1:10 # 冒号生成等差整数序列，含两端
seq(0, 1, by = 0.25) # seq() 更灵活：从 0 到 1，步长 0.25   Sequence  序列
rep(c("A", "B"), times = 2) # rep() 重复：times 指整体重复 2 次 → A B A B   repeat
rep(c("A", "B"), each = 2) # each 指每个元素各重复 2 次 → A A B B（和上一行对比着看）

length(v) # 向量长度（元素个数）→ 6    numeric
sum(v)
mean(v)
median(v)
sd(v) # 常用统计：和、均值、中位数、标准差
...
sum(v > 2) # 逻辑向量求和 = 统计 TRUE 的个数 → 4   # bool 布尔 逻辑 logical

any(v > 8)  # 有任意一个为真
all(v > 0) # 全部满足吗？常用来做断言检查
...
is.nan(b)
is.na(b) # is.nan() 只对 NaN 为真；is.na() 对 NaN 也返回 TRUE
...
t(m) # t() 转置：行列互换
rowSums(m)
colMeans(m) # 行和 / 列均值：内置函数比手写 for 循环快几十倍

L <- list(
  # list() 是"集装箱"：每个格子可装不同类型、不同长度
...
df_old <- data.frame(
  # 老式 data.frame：Base R 产物
  herb = c("人参", "黄芪", "当归"), # 字符列
  dose = c(9, 30, 12), # 数值列
  stringsAsFactors = FALSE # TRUE 会把字符列悄悄转成因子（R 4.x 默认已是 FALSE，老代码常见此参数）
)
class(df_old$herb) # 结果 factor —— 想做字符串处理却拿到因子，是最经典的坑之一
df_old # data.frame 打印时会把全部行都吐出来，行数多时刷屏

tb <- tibble(
  # tibble 是 tidyverse 的现代数据框（第 08 节的主角）
...
square(5) # 调用 → 25
range_stat <- function(x, na.rm = TRUE) {
  # 多行函数用 {}；参数可设默认值
...
for (i in 1:3) {
  print(i^2)
} # for 循环：能用向量化就别写循环，这里的 print 是副作用才需要循环
for (s in unique(iris$Species)) {
  cat(s, "有", sum(iris$Species == s), "条\n")
} # 遍历分类水平做统计；\n 是换行符
i <- 0 # while 循环前必须先初始化控制变量，否则死循环
while (i < 3) {
  i <- i + 1
  cat(i, "")
} # 条件为真时反复执行；同一行多个语句用分号分隔
...
read_csv(
  "data/01_base_tidyverse/iris_readr.csv", # 显式指定列类型：数据大时既能提速、又能避免类型误判
...
herbs <- tibble(
  # 构造一个 8 味中药的小数据集，后文所有 dplyr 演示都用它
...
herbs |>
  summarise(
    n = n(), # summarise() 汇总：n() 统计行数，结果是一张"1 行的小表"
    mean_dose = mean(max_dose), # 均值
    sd_dose = sd(max_dose), # 标准差
    max_paper = max(papers)
  ) # 最大值
...
herbs |>
  left_join(qi_ref, by = "qi") |> # 综合：拼上参考表（"微温"匹配不上 → nature 与 qi_order 都是 NA）
...
qi_stat <- herbs |>
  group_by(qi) |>
  summarise(papers_mean = mean(papers), dose_mean = mean(max_dose)) # 两个指标
qi_stat |>
  pivot_longer(
    cols = c(papers_mean, dose_mean),
    names_to = "metric",
    values_to = "value"
  ) # 宽 → 长
qi_long <- qi_stat |>
  pivot_longer(cols = -qi, names_to = "metric", values_to = "value") # cols = -qi：除 qi 外全部转长
...
if (interactive()) {
  print(p)
} # 交互式（RStudio）下在 Plot 面板显示；命令行批处理跳过 —— pdf 设备不支持中文字体
ggsave(
  "outputs/01_base_tidyverse/figures/qi_max_dose.png",
  p,
  width = 7,
  height = 4,
  dpi = 300
...
p2 <- ggplot(herbs, aes(x = max_dose, y = papers, label = herb)) + # 散点图：看"用药剂量"与"研究热度"的关系
  ...
  labs(x = "用量上限 (g)", y = "文献数", title = "用量与文献数分布") +
  theme_cn
if (interactive()) {
  print(p2)
} # 交互式下显示
ggsave(
  "outputs/01_base_tidyverse/figures/dose_vs_papers.png",
  p2,
  width = 7,
  height = 4,
  dpi = 300
...
p3 <- ggplot(qi_long, aes(x = qi, y = value, fill = metric)) + # 长表 + fill = 指标名 → 自动生成并列柱
  ...
  labs(x = "四气", y = "数值", fill = "指标") +
  theme_cn
if (interactive()) {
  print(p3)
} # 交互式下显示：同一四气下两个指标并排对比
